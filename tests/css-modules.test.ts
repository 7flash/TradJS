...
`,
    ...
    expect(result.css).toContain(
      `.${result.classMap["card"]} .${result.classMap["title"]}`,
    );
    expect(result.css).toContain(
      `.${result.classMap["list"]} > .${result.classMap["item"]}`,
    );
  });
...
`,
  ...
  test("throws on non-existent file", async () => {
    await expect(
      buildCSSModule("/nonexistent/file.module.css"),
    ).rejects.toThrow("CSS module not found");
  });
});
